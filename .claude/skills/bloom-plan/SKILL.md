---
name: bloom-plan
description: Decision-transparent planning skill using Cursor's Plan-Execute-Verify architecture as the brain. Produces structured plans with 3-4 alternatives and benchmarks for every decision, plus interactive HTML companions. Activate with /bloom-plan or /plan. Deactivate with /plan-off. Planning scale adapts automatically: Greenfield (full-scope), Brownfield (execution-level), or Phase Planning (task-level).
---
